...
class TargetPractice:
    def __init__(self, target_size=10):
        ...
        self.target_center = [random.randint(-50, 50), random.randint(-50, 50)]
        self.score = 0

    def calculate_distance(self, x, y):
        return math.sqrt((x - self.target_center[0])**2 + (y - self.target_center[1])**2)

    def display_target(self):
        print("Welcome to Target Practice Game!")
        print(f"Target Center: {self.target_center}")
        print("Try to hit the target!")

    def shoot(self, x, y):
        distance = self.calculate_distance(x, y)
        if distance < self.target_size:
            self.score += 10
            print(f"Hit! Distance to target: {distance:.2f}. Score: {self.score}")
        else:
            print(f"Miss! Distance to target: {distance:.2f}. Score: {self.score}")
...
if __name__ == "__main__":
    ...
    while True:
        try:
            x, y = map(int, input("Enter coordinates (x y) to shoot (press Enter to quit): ").split())
            game.shoot(x, y)
        except ValueError:
            print("Goodbye!" "\nTHANKS FOR PLAYING...")
            break
...
#     BTN_SUBMIT.pack()
#
#     LBL_GAME_DETAILS = tk.Label(master=FRM_USER_INFO,
